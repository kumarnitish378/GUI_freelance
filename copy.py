...
def run():
    ...
    def Effective_properties(Ef1, Ef2, Em, Vf):  # Defined Function to calculate effective properties
        ...
        data = data + ("--------------------------------------------------------------------------------------------\n")
        data = data + ('The Effective Properties of the lamina are:\n')
        data = data + ("--------------------------------------------------------------------------------------------\n")
        data = data + ("Longitudinal Stiffness of the composite, E1 (Gpa) =" + str(E1))
        data = data + ("\n")
        data = data + ("Transverse Stiffness of the lamina, E2 (Gpa) =" + str(E2))
        data = data + ("\n")
        data = data + ("Poisson's ratio of the lamina, nu12 =" + str(nu12))
        data = data + ("\n")
        data = data + ("Shear Modulus of the lamina, G12 (Gpa) ="+ str(G12))
        data = data + ("\n")
        data = data + ("--------------------------------------------------------------------------------------------\n")
    ...
    def stiffness_matrix(E1, E2, nu12, G12):  # Defined Function to calculate stiffness matrix
        ...
        # data = data + ("The Compliance Matrix is =\n\n",S_Matrix)
        # data = data + ("\n")
        # data = data + ("The Stiffness Matrix is =\n\n",Q)
        # data = data + ("\n")
        return Q
    ...
    def transformation_compliance(S, theta):  # Defined Function to calculate transformation of compliance matrix
        ...
        # data = data + ("Transformed Compliance Matrix is a general cordinate system is = \n", S_transformed)
        # data = data + ("\n")
    ...
    def transformation_stiffness(Q, theta):  # Defined Function to calculate transformation of stiffness matrix
        ...
        # data = data + ("The Transformed stiffness matrix in the X-Y coordinate system is = \n", Q_transformed)
        # data = data + ("\n")
    ...
    data = data + ("\n--------------------------------------------------------------------------------------------\n")
    data = data + ('______________________ABD Matrix is:________________________')
    data = data + ("\n" + str(ABD_Matrix_1))
    ...
    # data = data + (Sigma_K_store)
    # data = data + (theta_radian_store)
    ...
    def Hashin_Criteria(Xt, Xc, Yt, Yc, S12):
        for i in range(0, No_of_Layers):
            ...
            # data = data + (Condition_Matrix)
            # data = data + (Condition_Fiber)

            if (Condition_Matrix >= 0):
                # data = data + ("--------------------------------------------------------------------------------------------\n")
                # data = data + ("Checking Tensile failure mode in Matrix : \n")

                Failure_Value = (Sigma_22 / Yt ** 2) ** 2 + (Sigma_66 / S12) ** 2
                # data = data + ("Failure value is : \n", Failure_Value)

                if (Failure_Value >= 1):

                    # data = data + (f"Matrix of Ply {i+1} failed in Tensile Mode\n")
                    # data = data + (color.RED + color.BOLD + 'PLEASE CHECK' + color.END)
                    ...
                    # data = data + (f"Margin of safety for ply {i+1} is \n", R)

                    # data = data + (f"Matrix of Ply {i+1} is safe from Tensile Mode Failure\n")


            elif (Condition_Matrix < 0):
                # data = data + ("Checking Compressive failure mode in Matrix : \n")
                ...
                if (Failure_Value >= 1):
                    ...
                    # data = data + (f"Matrix of Ply {i+1} + \033[1m + failed in Compressive Mode\n")
                    # data = data + (color.RED + color.BOLD + 'PLEASE CHECK' + color.END)
                    ...
                    # data = data + (f"Margin of safety for ply {i+1} is \n", R)
                    # data = data + (f"Matrix of Ply {i+1} is safe from Compressive Mode Failure\n")
                    # data = data + ("--------------------------------------------------------------------------------------------\n")

            if (Condition_Fiber >= 0):

                # data = data + ("Checking Tensile failure mode in Fiber : \n")
                ...
                if (Failure_Value >= 1):
                    ...
                    # data = data + (f"Fibers of Ply {i+1} failed in Tensile Mode\n")
                    # data = data + (color.RED + color.BOLD + 'PLEASE CHECK' + color.END)
                    ...
                    # data = data + (f"Margin of safety for ply {i+1} is \n", R)
                    # data = data + (f"Fibers of Ply {i+1} is safe from Tensile Mode Failure\n")

            elif (Condition_Fiber < 0):
                # data = data + ("Checking Compressive failure mode in Fiber : \n")
                ...
                if (Failure_Value >= 1):
                    ...
                    # data = data + (f"Fibers of Ply {i+1} failed in Compressive Mode\n")
                    # data = data + (color.RED + color.BOLD + 'PLEASE CHECK' + color.END)
                    ...
                    # data = data + (f"Margin of safety for ply {i+1} is \n", R)
                    # data = data + (f"Fibers of Ply {i+1} is safe from Compressive Mode Failure\n")
        ...
        # data = data + (R_Final)
        data = data + ('------------RED color ------- Calculation of Margin of safety using Hashins Criteria')

        # for i in range(0, No_of_Layers):
        # data = data + (f"Margin of Safety for Ply {i+1} is: ", R_Final[i])
        # data = data + ("-------------------------------------------------------------------\n")

        data = data + ("---------------------------------------------------------------------\n")
        data = data + ("---------------------------------------------------------------------\n")
        data = data + ("Laminate no. ||  Ply Orientation ||  R_intact \n")
        data = data + ("+------------++------------------++-----------------------------------++\n")
        for i in range(0, No_of_Layers):
            data = data + (" || "+str(i + 1).ljust(5)+" || " + str(Ply_Orientation[i]).ljust(5)+ " || " + str(R_Final[i]).ljust(5))
            data = data + ("\n")
        data = data + ("+---------------------------------++-------------------------------------++\n")
        data = data + (f"Hence, for 0 degree ply the margin of safety is : " + str(R_Final[0])+"\n")
        data = data + (f"Hence, for 90 degree ply the margin of safety is :" + str(R_Final[3])+"\n")
        data = data + ("----------------------------------------------------------------------\n")
        data = data + ('Since the Margin of Safety for 90 degree laminate is less than 1, hence it has failed')
        data = data + ("\n---------------------------------------------------------------------\n")
        file.write(data)
        file.close()
...
def add():
    ...
    global Gm, Nx, Ny, Nxy, Mx, My, Mxy, Xt, Xc, Yt, Yc, S12, data
    try:
...
# file = open("result.txt", 'a')
# for i in range(10):
#     data = data + ("Shear Modulus of the composite, G12 =" + str(G54))
#     data = data + ("\n")
#
# data = data + ("________________________________________________________\n")
# file.close()
